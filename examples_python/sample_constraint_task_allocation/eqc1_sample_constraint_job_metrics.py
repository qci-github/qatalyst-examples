...
# 8) Get the sample_constraint job template
job_json = load_json_file("sample_constraint_job_template.json")

# 9) Edit the sample constraint job template. Attach the JSON data files 
# to the sample constraint job. Add the Lagrange parameter alpha.
...
# 10) Run the job
job_submit_response_json = qci.process_job(job_body=job_json, job_type="sample-constraint")
...
if job_submit_response_json['job_info']['details']['status'] == "COMPLETED":
    ...
    print(job_submit_response_json['job_info']['metrics']['provider']['qphoton']['time_ns']['wall']['total'])
    print(job_submit_response_json['job_info']['metrics']['provider']['qphoton']['time_ns']['wall']['dirac_1_controller'])
    assert results['energies'][0] == 13
else:
